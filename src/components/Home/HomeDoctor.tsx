...
const HomeDoctor = ({ id, rol }: Props) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:4700/doctors/ProfileDoc/${id}`)
  ...
  return (
    <div className="min-h-screen  bg-gray-100 flex flex-col items-center p-4 md:p-8">
        ...
        <div className="flex items-center w-[150px] space-x-4 border-b-2 gradient">
          ...
          <div className="flex flex-col p-0">
            <span className="font-semibold">
              {infoDoctor?.doctor.firstName} {infoDoctor?.doctor.lastName}
            </span>
        ...
        <div className="z-10">
          {doctor ? (
            doctor?.doctorInfoDate.map(
              (appointment: {
                _id: string;
                patient_id: { firstName: string; lastName: string };
                video_call_link: string;
                startTime: string;
                reasons: string;
              }): any => (
                <article
                  key={appointment._id}
                  className="flex text-[12px] font-bold justify-between items-center h-[50px] gap-6"
                >
                  <h3 className="self-start py-3">
                    {appointment.startTime} hr
                  </h3>
                  <ul className="flex flex-col text-[11px]">
                    ...
                    <li className="flex">Cita: {appointment.reasons}</li>
                  </ul>
      ...
      <Link
        href={`/${rol}/${id}`}
        className="bg-[#812b75] px-4 py-2 text-[15px] text-[#F2F2F2] font-semibold border rounded-xl mt-10"
      >
        Consultar Agenda
      </Link>
    </div>
